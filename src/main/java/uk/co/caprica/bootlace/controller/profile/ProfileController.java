 ...
 * <p>
 * Semantically a user profile (for a valid user) always "exists" even if it has not been created
 ...
 * authenticated user, i.e. this service does not allow for a particular profile to be retrieved
 * even if its unique identifier is known.
 * <p>
 * This is <em>not</em> typical either.
 * <p>
 * The profile information could just be kept with the user account information, but they are kept
 * separate so as not to blur responsibilities - i.e. the user account data is deemed "system"
 * data, whereas the profile is more like "application" data.
 * <p>
 * To simplify this a little bit, the profile used the same unique identifier as the user account.
 * <p>
